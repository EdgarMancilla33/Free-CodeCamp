...
def main():    
    ...
    def Editar_Contacto():
        def seleccionar_estudiante():
            ...
            if bandera == True:
                ...
                boton_actualizar = tk.Button(Ventana_Editar_Estudiante, text="Actualizar Contacto", command=actualizar_estudiante, bg='#800080', fg="white", font=("Arial", 12), width=15)
    ...
    def Eliminar_Contacto():
        ...
        def seleccionar_estudiante():
            ...
            print("Archivo Modificado")
            messagebox.showinfo(title="Contacto Eliminado", message=contacto_seleccionado)
...
# Crear listas con los datos para el gráfico
        ...
        def Jerarquica():
            ...
            with open("estudiantes.csv", "r") as file:
                lector = csv.reader(file)
                contactos = list(lector)
                fem=treeview.insert("",tk.END, text ="Femenino" )
                mas=treeview.insert("",tk.END, text ="Masculino" )
                for contacto in contactos:
                    print(contacto[4])
                    if contacto[4] == "Femenino":
                        treeview.insert(fem, tk.END, text=contacto[0])  
                    elif contacto[4] == "Masculino":
                        treeview.insert(mas, tk.END, text=contacto[0])
